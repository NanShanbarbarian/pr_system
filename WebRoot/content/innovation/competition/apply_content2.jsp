...
<body>
	<div class="main-content2">
		...
		<div class="appply_container" id="appply_container">
				<!-- <form action="competition_applyExcel.action" name="importForm" enctype="multipart/form-data" method="post"></form> -->
				...
				<div class="form-group">
	    			...
	    			<p class="help-block"><a href="attachment/竞赛获奖申请表.xlsx">附件：竞赛获奖申请表Excel文件</a></p>
	    			</div>
	    		</div>
		</div>
	</div>
	...
	<script type="text/javascript">
		...
		$('#file').fileinput({
				language: 'zh', //设置语言
				uploadUrl: 'competition_applyExcel.action',
		        ...
		        if(data.response == 10) {
		        	...
		        	alert("上传失败，上传时文件出现问题！");
		        } else if(data.response == 2) {
		        	alert("上传失败，文件格式规范错误或只能允许申请一次！");
		        } else if(data.response == 3) {
		        	alert("上传失败，竞赛名称填写不规范，请按照sheet2的规范填写！");
		        } else if(data.response == 4) {
		        	alert("上传失败，竞赛类别填写不规范，请按照sheet2的规范填写！！");
		        } else if(data.response == 5) {
		        	alert("上传失败，获奖层次填写不规范！");
		        } else if(data.response == 6) {
		        	alert("上传失败，获奖等级填写不规范！");
		        } else if(data.response == 7) {
		        	alert("上传失败，指导老师填写不规范！");
		        } else if(data.response == 8) {
		        	alert("上传失败，获奖时间填写不规范！");
		        } else if(data.response == 9) {
		        	alert("上传失败，获奖学年填写不规范！");
		        } else if(data.response == 11) {
		        	alert("上传失败，获奖学期填写不规范！");
		        }
			});
