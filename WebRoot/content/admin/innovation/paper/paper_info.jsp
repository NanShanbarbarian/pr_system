		...
		<div class="form-horizontal">
	    	...
	    	<div class="form-group">
	    		<div class="col-sm-4 col-sm-offset-5" style="margin-top: 30px;">
	    			<c:if test="${paper.isPass == 1}">
	    				<button class="btn btn-success" onclick="accept(${paper.id})">同意</button>
	    				<button class="btn btn-danger" onclick="refuse(${paper.id})">拒绝</button>
	    			</c:if>
	    			<c:if test="${paper.isPass == 0}">
	    				<a class="btn btn-success" href="content/admin/innovation/paper/paper.jsp" style="font-size: 13px;">确定返回</a>
	    				<button class="btn btn-danger" onclick="remove(${paper.id})">删除记录</button>
	    			</c:if>
	    		</div>
	    	</div>
		</div>
